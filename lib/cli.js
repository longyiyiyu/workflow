...
const { config, install, uninstall, list, init, build } = require('./api');
 ...
 * list 命令
 */
function initCmdList(callback) {
  program
    .command('list [name...]')
    .alias('ls')
    .description('to list plugins')
    .action(name => {
      const ret = list(name);

      if (ret && ret.length) {
        ret.forEach(({ name, version }) => {
 ...
 * init 命令
 */
function initCmdInit(callback) {
  program
    .command('init [name]')
    .description('to do some init work')
    .action(name => {
      init(name).then(ret => {
        callback(ret);
      });
 ...
 * dev 命令
 */
function initCmdDev(callback) {
  program
    .command('dev [name]')
    .description('to do some dev work')
    .action(name => {
      build(BUILD_TYPE_DEV, name).then(ret => {
        callback(ret);
      });
    });
}
 ...
 * build 命令
 */
function initCmdBuild(callback) {
  program
    .command('build [name]')
    .description('to do some build work')
    .action(name => {
      build(BUILD_TYPE_BUILD, name).then(ret => {
        callback(ret);
      });
    });
}
